...
flowchart TB
    ...
    subgraph MCP["MCP Tools Bridge (Experimental)"]
...
| Flag | Purpose |
|------|---------|
| `-e openclaw-tools` | Load MCP extension for OpenClaw coding tools (Experimental) |
...
## MCP Tools Bridge (Experimental)

OpenClaw exposes coding tools via an auto-generated `gemini-extension.json` manifest. The server implementation runs as a standalone process wrapping `src/agents/openclaw-tools.ts`.
